...
function AiToolModal({
  ...
  async function handleSubmit() {
    if (!form.name.trim()) { showToast('Tool-Name ist Pflichtfeld', 'error'); return; }
    if (form.url && !/^https?:\/\//i.test(form.url)) {
      showToast('URL muss mit https:// beginnen', 'error'); return;
    }
    const statusChanged = !tool || form.status !== tool.status;
    if (statusChanged && !form.justification.trim()) {
      showToast('Begründung ist bei Statusänderung Pflicht', 'error'); return;
    }
    ...
    try {
      if (tool) {
        ...
        showToast('Tool aktualisiert', 'success');
      } else {
        await createTool({ ...form, status: form.status as AiTool['status'] });
        showToast('✓ Tool angelegt', 'success');
      }
      onClose();
    } catch (err) {
  ...
  return (
    <Modal
      open={open}
      wide
      title={tool ? `${tool.id} — ${tool.name}` : 'Neues KI-Tool'}
      onClose={onClose}
      footer={
        readOnly ? (
          ...
          <>
            ...
            <button className="btn btn-primary" disabled={submitting} onClick={handleSubmit}>
              {submitting ? 'Speichert…' : tool ? 'Speichern' : 'Tool anlegen'}
            </button>
          </>
        )
      }
    >
      ...
      {tool && (
        <div className="tabs">
          ...
          <div className={`tab${tab === 'history' ? ' active' : ''}`} onClick={() => setTab('history')}>Historie</div>
        </div>
      )}
      ...
      {tab === 'details' && (
        <div className="fg">
          <div className="fgroup">
            {label('Tool-Name *')}
            <input value={form.name} disabled={readOnly} onChange={e => set('name', e.target.value)} placeholder="z.B. ChatGPT" />
          </div>
          <div className="fgroup">
            {label('Anbieter')}
            <input value={form.vendor} disabled={readOnly} onChange={e => set('vendor', e.target.value)} placeholder="z.B. OpenAI" />
          </div>
          <div className="fgroup">
            {label('Kategorie')}
          ...
          <div className="fgroup">
            ...
            {!isApprover && !readOnly && (
              <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 4 }}>
                Freigabe-Status (Erlaubt/Abgelehnt …) nur durch Approver/Admin
              </div>
            )}
          </div>
          <div className="fgroup full">
            {label('Begründung (Pflicht bei Statusänderung)')}
            <textarea rows={2} value={form.justification} disabled={readOnly}
              onChange={e => set('justification', e.target.value)}
              placeholder="Warum erlaubt / eingeschränkt / abgelehnt?" />
          </div>
          <div className="fgroup full">
            {label('Freigabe-Scope')}
            <textarea rows={2} value={form.scope} disabled={readOnly}
              onChange={e => set('scope', e.target.value)}
              placeholder="Für wen / mit welchen Datenklassen erlaubt (z.B. keine personenbezogenen Daten)" />
          </div>
          <div className="fgroup">
            {label('Datenstandort')}
          ...
          <div className="fgroup full">
            {label('Verknüpfte Use Cases (IDs, kommagetrennt)')}
      ...
      {tab === 'history' && (
        <div>
          {history === null ? (
            <div className="empty">Lade Historie…</div>
          ) : history.length === 0 ? (
...
export default function AiTools() {
  ...
  const filtered = useMemo(() => tools.filter(t => {
    ...
    if (search) {
      ...
      if (!(`${t.name} ${t.vendor}`.toLowerCase().includes(q))) return false;
    }
    return true;
  ...
  if (loading) return <div className="empty">Lade KI-Tools…</div>;

  return (
    <div>
      <div className="sec-title">Erlaubte AI-Tools</div>
      <div className="sec-sub">Register genehmigter KI-Tools inkl. Begründung und Änderungshistorie.</div>

      {/* Toolbar */}
      <div className="fb" style={{ marginBottom: 16 }}>
        <input placeholder="Suche Name/Anbieter…" value={search} onChange={e => setSearch(e.target.value)} style={{ minWidth: 180 }} />
        ...
        {(statusF || catF || search) && (
          <button className="btn btn-outline btn-sm" onClick={() => { setStatusF(''); setCatF(''); setSearch(''); }}>✕ Filter</button>
        )}
        <div style={{ flex: 1 }} />
        <button className="btn btn-outline btn-sm" disabled={!filtered.length} onClick={() => exportAiToolsCSV(filtered)}>⬇ CSV</button>
        {isEditor && <button className="btn btn-primary" onClick={openNew}>➕ Tool hinzufügen</button>}
      </div>
      ...
      <div className="card">
        <div className="table-wrap">
          <table>
            <thead>
              <tr>
                <th>Tool</th><th>Kategorie</th><th>Status</th>
                <th>Datenstandort</th><th>AVV</th><th>Review</th>
              </tr>
            </thead>
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={6} className="empty">Keine Tools gefunden.</td></tr>
              ) : filtered.map(t => (
                <tr key={t.id} className="clickable" onClick={() => openTool(t)}>
                  <td>
                    <div style={{ fontWeight: 600 }}>{t.name}</div>
                    <div style={{ fontSize: 11, color: 'var(--muted)' }}>{t.vendor || '—'} · {t.id}</div>
                  </td>
                  <td>{t.category}</td>
                  <td><span className={`badge ${AITOOL_STATUS_CSS[t.status] ?? 'bgr'}`}>{t.status}</span></td>
                  <td>{t.dataLocation}</td>
                  <td>{t.dpa ? '✓' : '—'}</td>
                  <td>
                    {t.reviewDate ? new Date(t.reviewDate).toLocaleDateString('de-DE') : '—'}
                    {reviewOverdue(t.reviewDate) && <span className="badge br" style={{ marginLeft: 6 }}>Review fällig</span>}
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <div style={{ marginTop: 12, fontSize: 12, color: 'var(--muted)', textAlign: 'right' }}>
        {filtered.length !== tools.length ? `${filtered.length} von ${tools.length} Tools` : `Gesamt: ${tools.length} Tools`}
      </div>
